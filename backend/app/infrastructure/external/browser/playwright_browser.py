...
class PlaywrightBrowser:
    ...
    async def _extract_interactive_elements(self) -> List[str]:
        ...
        interactive_elements = await self.page.evaluate("""() => {
            ...
            for (let i = 0; i < elements.length; i++) {
                ...
                // Only add data-lads-id attribute to elements that meet the conditions
                element.setAttribute('data-lads-id', `lads-element-${validElementIndex}`);
                                                        
                // Build selector - using only data-lads-id
                const selector = `[data-lads-id="lads-element-${validElementIndex}"]`;
    ...
    async def _get_element_by_index(self, index: int) -> Optional[Any]:
        """Get element by index using data-lads-id selector
        ...
        # Use data-lads-id selector
        selector = f'[data-lads-id="lads-element-{index}"]'
